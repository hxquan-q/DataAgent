...
export type OpenSseOptions = {
	...
	/** HTTP/network errors only — not called on clean stream end */
	onError?: (error: Error) => void | Promise<void>;
	/** Stream body finished (after last message) */
...
export function openSseStream(url: string, options: OpenSseOptions): () => void {
	...
	void (async () => {
		try {
			...
			if (!res.body) {
				if (options.onError) await options.onError(new Error('浏览器不支持流式响应'));
				return;
			}
			...
			await flush();
			if (!closed && options.onDone) await options.onDone();
		} catch (e) {
