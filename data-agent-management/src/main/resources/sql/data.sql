...
-- v0.1 内置报告技能预设（#15）：按 agent 绑定后注入报告 prompt 的 {optimization_section}
INSERT IGNORE INTO `skill` (`id`, `name`, `description`, `scope`, `triggers`, `content`, `params_json`, `enabled`, `priority`, `display_order`, `create_time`, `update_time`) VALUES
(1, '管理驾驶舱', '面向决策层的概览报告：结论先行、KPI 优先、图表为主', 'report', '驾驶舱,KPI,概览,汇报', '报告风格：管理驾驶舱。\n结构要求：\n1) 开头给出 3-5 条要点摘要，每条含一个关键 KPI 及同比/环比；\n2) 优先使用指标卡、趋势折线、占比饼图，尽量少用大表；\n3) 每个结论后跟一句业务解读；\n4) 结尾给出 2-3 条可执行建议。\n语气：简洁、结论先行、面向决策层。', NULL, 1, 100, 1, NOW(), NOW()),
(2, '深度分析', '面向业务复盘的深度报告：论证链、多维交叉、归因与建议', 'report', '深度,分析,归因,复盘,洞察', '报告风格：深度分析。\n结构要求：\n1) 背景与问题定义；\n2) 多步骤分析过程，含关键 SQL/Python 逻辑说明；\n3) 多维度交叉图表（趋势、结构、相关性）；\n4) 洞察与归因；\n5) 局限性与后续建议。\n语气：严谨、有论证链、面向业务复盘。', NULL, 1, 80, 2, NOW(), NOW()),
(3, '数据探索', '面向分析师的探索报告：明细优先、维度拆解、标注异常', 'report', '明细,探索,下钻,分布,异常', '报告风格：数据探索。\n结构要求：\n1) 先展示完整结果表（分页或 Top-N），保留明细；\n2) 按维度拆解，每个维度配一张对比图；\n3) 标注异常值与分布特征；\n4) 提供进一步下钻方向。\n语气：客观、注重数据完整性、面向分析师。', NULL, 1, 50, 3, NOW(), NOW());

-- R2: 全局报告高密度优化（agent_id NULL = 全智能体默认）
INSERT IGNORE INTO `user_prompt_config` (`id`, `name`, `prompt_type`, `agent_id`, `system_prompt`, `enabled`, `description`, `priority`, `display_order`, `creator`) VALUES
('global-report-dense-v1', '全局默认·报告高密度', 'report-generator', NULL,
 '结论先行：直接回答用户问题，含关键数字与单位。信息密度优先，默认 80–250 字；禁止背景/过程回顾/后续行动等注水段。不得杜撰数据；无数据时一句话说明。图表仅在有助于理解时使用，且必须是 echarts 纯 JSON 代码块。',
 1, '全局质量兜底（agent_id NULL 对所有智能体生效）', 1000, 0, 'iteration-loop');
